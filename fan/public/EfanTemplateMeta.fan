...
const class EfanTemplateMeta {
	...
	** Name of the rendering method on 'type'.
	const Str renderMethodName
	...
	** Any errs thrown are wrapped in an 'EfanErr' that shows where in the efan template the error occurred.
	Str renderFrom(Obj instance, Obj? ctx) {
		...
		try	return type.method(renderMethodName).call(instance, ctx)
		catch (Err err)
	...
	** If the given err can not be converted, it is returned.
	** 
	** For advanced use only. Normal users should just call 'render()' instead.  
	Err efanRuntimeErr(Err cause) {
		regex 	:= Regex.fromStr("^\\s*?${type.qname}\\.${renderMethodName}\\s\\(${type.pod.name}:([0-9]+)\\)\$")
